...
;; Implementation of and procedure
;; Returns true if no arguments are given
;; Returns false if any of the given arguments are false
;; Otherise returns value from last argument
(define (and-1 exp) 
  (cond ((= (length exp) 0) #t)
...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ; Excersize 7
    ((and-exp? exp)             ; check if and procedure is being called
     (displayln "and-exp? is true")
     (and-1 (cdr exp)))         ; evaluate using custom implementation
    ((symbol? exp) (eval exp))  ; use underlying Racket's EVAL
    ; Excersize 5
    ((quote-exp? exp)
         (displayln "quote? is true")
         (if (> (length exp) 2)(error "quote: bad syntax ") (cadr exp))) ; return error if bad number of arguments are given
    ((if-exp? exp)
     (if (eval-1 (cadr exp))
         (eval-1 (caddr exp))
         (eval-1 (cadddr exp))))
    ; Excersize 6
    ((map-exp? exp)                           ; check if map procedure is being called
     (displayln "map-exp? is true")
     (apply-1 map (map eval-1 (cdr exp))))    ; evaluate
    ((lambda-exp? exp) (eval exp))            ; Also changed for excersize 6
